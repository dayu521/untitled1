...
#define TSTR(s)                                                                \
  ([] {                                                                        \
    ...
    return Ubpa::detail::TStr<                                                 \
      Ubpa::detail::fcstr<typename decltype(str)::value_type, str.size()>{     \
        str }>{};                                                              \
  }())
...
FindIf(const L&, F&&, std::index_sequence<>)
{
  return -1;
}
template<class L, class F, std::size_t N0, std::size_t... Ns>
constexpr std::size_t
FindIf(const L& l, F&& f, std::index_sequence<N0, Ns...>)
{
...
constexpr auto
Acc(const L&, F&&, R r, std::index_sequence<>)
{
  return r;
}
template<class L, class F, class R, std::size_t N0, std::size_t... Ns>
constexpr auto
Acc(const L& l, F&& f, R r, std::index_sequence<N0, Ns...>)
{
  return Acc(l,
             std::forward<F>(f),
...
struct ElemList
{
  ...
  constexpr std::size_t FindIf(F&& f) const
  {
    return detail::FindIf(
      *this, std::forward<F>(f), std::make_index_sequence<size>{});
  }
  template<class S>
...
struct TypeInfoBase
{
  ...
  static constexpr void ForEachVarOf(U&& obj, Func&& func)
  {
    ...
    detail::NV_Var(
      TypeInfo<Type>{}, std::forward<U>(obj), std::forward<Func>(func));
  }
};
template<class Name>
