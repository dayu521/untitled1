...
inline Error TypeHandler<T>::to(T &to_type, ParseContext &context)
{
  ...
  if (error != JS::Error::NoError)
    return error;
  //调用JsonStructBase<JS_OBJECT_T>::js_static_meta_data_info()
  //返回
  /*JS::makeTuple(
  MI=JS::makeMemberInfo(#member1, &JS_OBJECT_T::member1),
  MI=JS::makeMemberInfo(#member2, &JS_OBJECT_T::member2))*/
  ...
  bool assigned_members[Internal::memberCount<T, 0>()];//当前成员数量与基类成员数量总和
  memset(assigned_members, 0, sizeof(assigned_members));    //都是false
